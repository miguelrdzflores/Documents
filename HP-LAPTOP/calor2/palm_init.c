...
/* file created by prepalm_mp user miguel lun ago  8 18:07:36 CDT 2016 */
...
char *cf_ppalm_os(char *cla_uname, char *cla_oname)
{
   ...
   if (strcmp(cla_uname, "imprime" ) == 0 )  {
      if (strcmp(cla_oname, "matriz.imprime" ) == 0 )  return("mat1d.imprime");
   }
   return NULL;
}
  ...
  switch (il_num)
    {
     ...
     case 1:
         /* main_imprime */
          cl_nbproc = getenv("NBPROC_main_imprime");
          if (cl_nbproc == NULL) {
               return 1;
          } else {
               return atoi(cl_nbproc);
...
{
   static char *cl_name[] = {"palm_main","main_calor1d","main_imprime"};
   return cl_name;
}
...
{
   static char *cl_name[] = {"calor1d","imprime"};
   return cl_name;
}
...
int if_ppalm_exenum_for_entname(char *cd_entname)
{
   ...
   /* imprime ---> 1 ---> 1 */
   if (strcmp("imprime",cd_entname) == 0) return 1;
...
int if_ppalm_branch_for_entname(char *cd_entname)
{
   ...
   /* imprime ---> 1 */
   if (strcmp("main_imprime",cd_entname) == 0) return 1;
