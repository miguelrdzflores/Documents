...
!
!**** module m_ppalm_driver_b0 : private variable for branch b1
!
!     Purpose:
!     --------
!
...
module m_ppalm_driver_b1
  use palmlib
  use palm_user_param
  
!
!**** module m_ppalm_driver_b1 : private variable for branch b2
!
!     Purpose:
!     --------
!
   ...
   select case (id_linesw) 
     ...
     !Get object b1_get_1 
        ...
        if (il_ppalm_rank .eq. 0) then 
           call PALM_get(cl_ppalm_space, cl_ppalm_name, il_ppalm_time, il_ppalm_tag, somme, il_ppalm_err)
           call MPI_Bcast(somme, 1, MPI_INTEGER, 0, MPI_COMM_WORLD, il_ppalm_err) 
        else 
           call MPI_Bcast(somme, 1, MPI_INTEGER, 0, MPI_COMM_WORLD, il_ppalm_err) 
        endif 
!        if (il_ppalm_err .ne. 0) then
!            print *,'Error in PALM_get(one_integer,b1_get_1,PL_NO_TIME,PL_NO_TAG) '
...
!        endif
     ...
     !Put object b1_put_1 
        ...
        call MPI_Comm_rank(MPI_COMM_WORLD,il_ppalm_rank,il_ppalm_err)
        if (il_ppalm_rank .eq. 0) then 
           call PALM_Put(cl_ppalm_space, cl_ppalm_name, il_ppalm_time, il_ppalm_tag, somme, il_ppalm_err)
        endif 
!        if (il_ppalm_err .ne. 0) then
...
!        if (il_ppalm_err .ne. 0) then
!            print *,'Error in PALM_get(one_integer,b2_get_1,PL_NO_TIME,PL_NO_TAG) '
!            il_ppalm_err2 = il_ppalm_err
!            call PALM_Error_explain(il_ppalm_err, il_ppalm_err)
!            call PALM_Abort(il_ppalm_err)
!        endif
...
!        if (il_ppalm_err .ne. 0) then
!            print *,'Error in PALM_Put(one_integer,b2_put_1,PL_NO_TIME,PL_NO_TAG) '
!            il_ppalm_err2 = il_ppalm_err
!            call PALM_Error_explain(il_ppalm_err, il_ppalm_err)
!            call PALM_Abort(il_ppalm_err)
!        endif
...
!                     id_way = 2 recv module and setup variables
   integer :: il_err
    ...
    integer, dimension(2) :: ila_mess
!
   if (id_way.eq.1) then
       ila_mess(1) = somme
       ila_mess(2) = n

       call if_intermpisend(IP_PORT_ENT,id_bloc,ila_mess,2*KIND(ila_mess(1)),id_proc,1238,il_err)
   endif
 
   if (id_way.eq.2) then
       call if_intermpirecv(IP_PORT_ENT,id_bloc,ila_mess,2*KIND(ila_mess(1)),id_proc,1238,il_err)
       somme = ila_mess(1) 
       n = ila_mess(2) 
...
!
   select case (id_unit) 
          case (2)
!              main_je_compte
               call getenv('NBPROC_main_je_compte',cl_nbproc)
...
!         block 1
          case (4)
               il_maxi_proooc = 1
               if ((1) .gt. il_maxi_proooc) il_maxi_proooc = 1
!              main_block_1
               call getenv('NBPROC_main_block_1',cl_nbproc)
               if (cl_nbproc(1:3) .eq. '   ') then 
                   id_value = il_maxi_proooc
               else
                    read (cl_nbproc,*) id_value
               endif
               return
...
integer function if_ppalm_steptimetag_idx(id_numfunc,id_time,id_tag)
   ...
   integer if_ppalm_e_nb_steptime_0
   integer if_ppalm_e_steptime_0
   integer if_ppalm_e_steptag_0
 
   if_ppalm_steptimetag_idx = -1
 
   select case (id_numfunc) 
       case (0)
            i  = if_ppalm_e_steptime_0(id_time)
            if (i .eq. -1) return
            j  = if_ppalm_e_steptag_0(id_tag)
            if (j .eq. -1) return
            nx = if_ppalm_e_nb_steptime_0()
            if_ppalm_steptimetag_idx = i + (j-1)*nx
            return
   end select
...
integer function if_ppalm_steptimetag_nb(id_numfunc)
   ...
   integer if_ppalm_e_nb_steptime_0
   integer if_ppalm_e_nb_steptag_0
 
   if_ppalm_steptimetag_nb = -1
 
   select case (id_numfunc) 
       case (0)
            if_ppalm_steptimetag_nb = if_ppalm_e_nb_steptime_0() * if_ppalm_e_nb_steptag_0()
            return
   end select
...
integer function if_ppalm_e_steptime_0(id_z_in)
!  
   use palm_user_param
!  
   implicit none
   integer, intent(in)    :: id_z_in
 
   integer :: il_z_out
   integer :: il_z_nbtot,il_z_cpt,il_z_res
!  
   integer if_ppalm_se_steptime_0_1
 
   il_z_cpt = 0
   if_ppalm_e_steptime_0 = -1
  
   il_z_res = if_ppalm_se_steptime_0_1(il_z_cpt,id_z_in,il_z_out,il_z_nbtot)
   if (il_z_res.ne.-1) then
       if_ppalm_e_steptime_0 = il_z_out 
       return
   endif
   il_z_cpt = il_z_cpt + il_z_nbtot
end function if_ppalm_e_steptime_0 
 
integer function if_ppalm_e_nb_steptime_0()
   use palm_user_param
   ...
   il_max = il_max + 1
   if_ppalm_e_nb_steptime_0 = il_max
 
end function if_ppalm_e_nb_steptime_0

integer function if_ppalm_se_steptime_0_1(id_z_cpt,id_z_in,id_z_out,id_z_nbtot)
!  evaluate PL_NO_TIME 
!  id_z_cpt : numero a partir duquel on compte (permet de traiter plusieurs sequences
!  id_z_in  : temps ou tag recherche
!  id_z_out : en sortie : numero d'ordre de l'element trouve
!  id_z_nbtot : en sortie : nombre total d'elements valide pour l'expression
!  
!  en sortie, la fonction vaut -1 si id_z_in ne fait pas partie de l'expression
!  
...
!  
   integer, intent(in)    :: id_z_in
   integer, intent(inout) :: id_z_cpt
   integer, intent(out)   :: id_z_out,id_z_nbtot
   ...
   id_z_nbtot = 1
   if_ppalm_se_steptime_0_1 = id_z_out
end function if_ppalm_se_steptime_0_1
 
 
integer function if_ppalm_e_steptag_0(id_z_in)
!  
   use palm_user_param
!  
   implicit none
   ...
   integer :: il_z_nbtot,il_z_cpt,il_z_res
!  
   integer if_ppalm_se_steptag_0_1
 
   il_z_cpt = 0
   if_ppalm_e_steptag_0 = -1
  
   il_z_res = if_ppalm_se_steptag_0_1(il_z_cpt,id_z_in,il_z_out,il_z_nbtot)
   if (il_z_res.ne.-1) then
       if_ppalm_e_steptag_0 = il_z_out 
       return
   endif
   il_z_cpt = il_z_cpt + il_z_nbtot
end function if_ppalm_e_steptag_0 
 
integer function if_ppalm_e_nb_steptag_0()
   use palm_user_param
   ...
   il_max = il_max + 1
   if_ppalm_e_nb_steptag_0 = il_max
 
end function if_ppalm_e_nb_steptag_0

integer function if_ppalm_se_steptag_0_1(id_z_cpt,id_z_in,id_z_out,id_z_nbtot)
!  evaluate PL_NO_TAG 
!  id_z_cpt : numero a partir duquel on compte (permet de traiter plusieurs sequences
!  id_z_in  : temps ou tag recherche
!  id_z_out : en sortie : numero d'ordre de l'element trouve
...
!  
   ...
   id_z_nbtot = 1
   if_ppalm_se_steptag_0_1 = id_z_out
end function if_ppalm_se_steptag_0_1
...
!
   select case (cd_name) 
     case ('n.je_compte')
        if_ppalm_b0_setget = 1
        call c_vf_ppalm_b0_sg_t0(id_val)
        return
...
subroutine vf_ppalm_b0_sg_t0(id_val)
...
!
   id_val = n
!
end subroutine vf_ppalm_b0_sg_t0
